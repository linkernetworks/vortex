...
.PHONY: apps.launch
apps.launch:
	yq r deploy/helm/config/develop.yaml services | helm install --name vortex-services --debug --wait -f - deploy/helm/services
	yq r deploy/helm/config/develop.yaml apps | helm install --name vortex-apps --debug --wait -f - --set vortex-server.controller.apiserverImageTag=$(SERVER_VERSION) deploy/helm/apps

.PHONY: apps.upgrade
apps.upgrade:
	yq r deploy/helm/config/develop.yaml services | helm upgrade vortex-services --debug -f - deploy/helm/services
	yq r deploy/helm/config/develop.yaml apps | helm  upgrade vortex-apps --debug -f - --set vortex-server.controller.apiserverImageTag=$(SERVER_VERSION) deploy/helm/apps
