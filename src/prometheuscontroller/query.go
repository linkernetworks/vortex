...
func queryRange(sp *serviceprovider.Container, expression string, timeGap time.Duration, queryResolution time.Duration) (model.Matrix, error) {
	api := sp.Prometheus.API

	rangeSet := pv1.Range{Start: time.Now().Add(-timeGap), End: time.Now(), Step: queryResolution}
